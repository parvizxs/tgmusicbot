...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text('Oyunçu boşdur')
    temp = []
    for t in queue:
        temp.append(t)
...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        ...
        if len(que) > 0:
            ...
            stats += 'Növbə : `{}`\n'.format(len(que))
            stats += 'İndi Oynanır : **{}**\n'.format(queue[0][0])
            stats += 'Tərəfindən oxunur : {}'.format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply('Bu söhbətdə çalışan VoiceChat nümunələri yoxdur')
    ...
    if type_ == 'pause':
        ...
        if (
            chat_id not in callsmusic.pytgcalls.active_calls
                ...
                await cb.answer('Çat bağlıdır və ya artıq çalınmır', show_alert=True)
        else:
            callsmusic.pytgcalls.resume_stream(chat_id)
        ...
        if chat_id not in callsmusic.pytgcalls.active_calls:
            ...
            if callsmusic.queues.is_empty(chat_id):
                ...
                await cb.message.edit('- Daha Playlist yoxdur..\n- VC -dən ayrılır!')
            else:
                ...
                await cb.message.reply_text(f'- Atlanan trek\n- İndi Oynanır **{qeue[0][0]}**')
        ...
        if chat_id in callsmusic.pytgcalls.active_calls:
            ...
            await cb.answer('Çat bağlı deyil!', show_alert=True)

@Client.on_message(command("play") & other_filters)
async def play(_, message: Message):
    global que
    lel = await message.reply("**__İşlənilir__**")
           ...
           for administrator in administrators:
                      if administrator == message.from_user.id:  
                          try:
                              ...
                              await lel.edit(
                                  "<b>Məni əvvəlcə qrupa admin olaraq əlavə edin</b>",
                              )
                              return
                          ...
                          try:
                              ...
                              #print(e)
                              await lel.edit(
                                  f"<b>🔴 Gözləmə xətası 🔴 \nİstifadəçi {user.first_name} userbot üçün çoxlu istəklər olduğu üçün qrupunuza qoşula bilmədi! İstifadəçinin qrupda qadağan edilmədiyinə əmin olun."
    ...
    try:
        ...
        await lel.edit(
            f"<i> {user.first_name} İstifadəçi botu bu söhbətdə deyil, Admindən ilk dəfə əmr göndərməsini /oynatmasını və ya {user.first_name} -ni əl ilə əlavə etməsini xahiş edin.</i>"
        )
        return     
    sender_id = message.from_user.id
    sender_name = message.from_user.first_name
    await lel.edit("**__Mahnını Axtarıram__**")
    sender_id = message.from_user.id
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("**__İşə salınır..__**")
    ...
    try:
        ...
        await lel.edit("Mahnı tapılmadı.Başqa bir mahnı sınayın və ya düzgün yazın.")
        print(str(e))
        return
    ...
    keyboard = InlineKeyboardMarkup(
            [   
                ...
                [
                    ...
                    InlineKeyboardButton('Çıxış 🗑', callback_data='cls')
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Oxunur** Burada {} tərəfindən İstənilən Mahnı".format(
