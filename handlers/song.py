...
def song(client, message):
    ...
    try:
        ...
        m.edit(
            "❌ Heç nə tapmadı.\n\nBaşqa bir əsas şey sınayın və ya düzgün yazın."
        )
        print(str(e))
        return
...
async def jssong(_, message):
    ...
    if len(message.command) < 2:
        await message.reply_text("/saavn arqument tələb edir.")
        return
    if is_downloading:
        await message.reply_text("Başqa bir yükləmə davam edir, bir müddət sonra yenidən cəhd edin.")
        return
    is_downloading = True
...
async def deezsong(_, message):
    global is_downloading
    if len(message.command) < 2:
        await message.reply_text("/deezer arqument tələb edir.")
        return
    if is_downloading:
        await message.reply_text("Başqa bir yükləmə davam edir, bir müddət sonra yenidən cəhd edin.")
        return
...
async def ytmusic(client,message: Message):
    ...
    pablo =  await client.send_message(
            message.chat.id,
            f"`Youtube serverlərindən {urlissed} alınır. Zəhmət olmasa, gözləyin.`")
    if not urlissed:
    ...
    try:
        ...
        with youtube_dl.YoutubeDL(opts) as ytdl:
            ...
            if duration > 8:
                await pablo.edit(
                    f"❌ 8 dəqiqədən uzun videolara icazə verilmir, təqdim olunan video {müddəti} dəqiqədir"
                )
                is_downloading = False
                return
        ...
        #await pablo.edit(event, f"**Yüklənmədi** \n**Error :** `{str(e)}`")
        is_downloading = False
        return
    
    c_time = time.time()
    file_stark = f"{ytdl_data['id']}.mp4"
    capy = f"**Video adı ➠** `{thum}` \n**yüklənmək üçün tələb olunur :** `{urlissed}` \n**Channel :** `{thums}` \n**Link :** `{mo}`"
